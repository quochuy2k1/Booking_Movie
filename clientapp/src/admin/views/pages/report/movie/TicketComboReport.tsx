...
import { Column, ColumnFixing, FilterBuilderPopup, FilterPanel, LoadPanel, FilterRow, GroupPanel, Grouping, HeaderFilter, Item, Pager, Paging, SearchPanel, Selection, Format, Export, Summary, TotalItem } from "devextreme-react/data-grid";
...
const TicketComboReport: React.FC = () => {
    ...
    useEffect(() => {
        const data = createStore({
            key: 'id',
            loadMethod: "POST",
            loadUrl: `${url}/api/Movies/movie-ticket-combo-report`,
            loadParams: {
                ...filter
            },
        ...
        exportDataGrid({
            ...
            customizeCell: ({ gridCell, excelCell }) => {
                ...
                if (gridCell!.rowType === "totalFooter" ) {
                    ...
                    excelCell!.font = { size: 13, bold: true}
            ...
            //1
            worksheet.mergeCells(footerRowIndex, 2, footerRowIndex, 2);
            footerRow.getCell(2).value = 'Người lập';
            footerRow.getCell(2).font = { bold: true, size: 13 };
            footerRow.getCell(2).alignment = { horizontal: 'center' };
            worksheet.mergeCells(footerRowIndex + 1, 2, footerRowIndex + 1, 2);
            footerRow2.getCell(2).value = '(Chữ ký/Họ tên)';
            footerRow2.getCell(2).font = { italic: true, size: 10 };
            footerRow2.getCell(2).alignment = { horizontal: 'center' };
            ...
            //2
            worksheet.mergeCells(footerRowIndex, 4, footerRowIndex, 4);
            footerRow.getCell(4).value = 'Kế toán trưởng';
            footerRow.getCell(4).font = { bold: true, size: 14 };
            footerRow.getCell(4).alignment = { horizontal: 'center' };
            worksheet.mergeCells(footerRowIndex + 1, 4, footerRowIndex + 1, 4);
            ...
            //3
            worksheet.mergeCells(footerRowIndex, 6, footerRowIndex, 6);
            footerRow.getCell(6).value = 'Giám đốc';
            footerRow.getCell(6).font = { bold: true, size: 13 };
            footerRow.getCell(6).alignment = { horizontal: 'center' };
            worksheet.mergeCells(footerRowIndex + 1, 6, footerRowIndex + 1, 6);
            footerRow2.getCell(6).value = '(Chữ ký/Họ tên)';
            footerRow2.getCell(6).font = { italic: true, size: 10 };
            footerRow2.getCell(6).alignment = { horizontal: 'center' };
            ...
            for (let col = 1; col <= 10; col++) {
                ...
                cell.font = { color: { argb: '333333' }, bold: true, size: 14 }
            }
                ...
                >
                    ...
                    >
                        <Column
                            ...
                            calculateDisplayValue={(dataRow: any) => {
                                return dataRow.ticketName
                            }}
                            calculateFilterExpression={(filterValue: any, selectedFilterOperation: string, target: string) => {
                                if (target === "filterRow") {
                                    return [["ticketName", selectedFilterOperation, filterValue]];
                                }
                                return null;
                            }}
                        />
                        <Column
                            dataField="ticketQuantityTotal"
                            caption={"Tổng sl vé"}
                            dataType="number"
...
export default TicketComboReport;